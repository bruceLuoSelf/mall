...
import com.wzitech.gamegold.rc8.service.servicer.ICustomServicer;
...
public class CustomServicerImpl extends AbstractBaseService implements ICustomServicer {
    ...
    public LoginResponse login(@QueryParam("") LoginRequest loginRequest,
        ...
        try {
            ...
            String decPwd = DESHelper.decrypt(loginRequest.getPwd(), encryptKey);

            // 校验MD5
            String toEncrypt = EncryptHelper.md5(
                    String.format("%s_%s_%s_%s_%s", name, decPwd, loginRequest.getMovepwd(), loginRequest.getVersion(), encryptKey)
            );
