...
public class RepositoryDBDAOImpl extends AbstractMyBatisDAO<RepositoryInfo, Long> implements IRepositoryDBDAO {
    ...
    /**
     * 此查询仅适用于地下城与勇士  金币 跨区跨服查询 参数经过processRepositoryTransfer 方法处理 query有变化
     *  去除了server servers唯一性限制 跨区合服需要在商城开启服务才可 对应查询
     */
    @Override
    public List<RepositoryInfo> queryMinUnitPriceRepository(Map<String, Object> query, int start, int pageSize) {
        return this.getSqlSession().selectList(this.mapperNamespace + ".selectMinPriceRepositoryFrom", query, new RowBounds(start, pageSize));
    }
    ...
    @Override
    public int countUniqueAndOldSellerRepository(Map<String, Object> queryParam) {
        return (Integer) this.getSqlSession().selectOne(this.mapperNamespace + ".countUniqueAndOldRepositoryByMap", queryParam);
    }

    @Override
    public List<RepositoryInfo> queryUniqueAndOldSellerRepository(Map<String, Object> queryMap) {
        return this.getSqlSession().selectList(this.mapperNamespace + ".queryUniqueAndOldSellerRepository", queryMap);
    }
    ...
    @Override
    public List<RepositoryInfo> selectMINunitPriceByOtherGameForCross(Map<String, Object> queryParam) {
        return this.getSqlSession().selectList(this.mapperNamespace + ".selectMINunitPriceByOtherGameForCross", queryParam);
    }
     ...
     */
    @Override
    public long selectSalableCount(Map<String, Object> queryMap) {
        Validate.notEmpty(queryMap);
        Long count = this.getSqlSession().selectOne(this.mapperNamespace + ".selectSalableCount", queryMap);
     ...
     */
    @Override
    public List<RepositoryInfo> selectSalableRepository(Map<String, Object> queryParam, List<SortField> sortFields) {
        if (null == queryParam) {
            queryParam = new HashMap<String, Object>();
        }
        String orderByStr = "";
        ...
        queryParam.put("orderBy", orderByStr);
        return this.getSqlSession().selectList(this.mapperNamespace + ".selectSalableRepository", queryParam);
    }
     ...
     */
    @Override
    public GenericPage<RepositoryInfo> selectSalableRepositoryForPage(Map<String, Object> queryParam, int pageSize, int startIndex, String orderBy, boolean isAsc) {
        ...
        if(pageSize < 1) {
            ...
            int totalSize = this.countByMapForSalableRepository((Map)queryParam);
            if(totalSize == 0) {
                return new GenericPage((long)startIndex, (long)totalSize, (long)pageSize, (List)null);
            } else {
                List pagedData = this.getSqlSession().selectList(this.mapperNamespace + ".selectSalableRepository", queryParam, new RowBounds(startIndex, pageSize));
    ...
    private int countByMapForSalableRepository(Map queryParam) {
        return ((Integer)this.getSqlSession().selectOne(this.mapperNamespace + ".countByMapForSalableRepository", queryParam)).intValue();
    }
}
