...
import com.wzitech.gamegold.rc8.service.order.IOrderService;
...
public class OrderServiceImpl extends AbstractBaseService implements IOrderService {
    ...
    private static String reasonXml;
    ...
    public QueryOrderListResponse queryOrderList(@QueryParam("") QueryOrderListRequest request) {
        ...
        try {
            String decPwd = DESHelper.decrypt(request.getPwd(), encryptKey);

            // 校验MD5
            String toEncrypt = EncryptHelper.md5(
                    String.format("%s_%s_%s", request.getName(), decPwd, encryptKey)
            );
    ...
    public String queryOrderInfo(@PathParam("orderId") String orderId,
        ...
        try {
            ...
            if (!StringUtils.equals(toEncrypt, request.getSign())) {
                ...
                logger.info("xml:{}", xml);
                return DESHelper.encrypt(xml, encryptKey);
            }
            ...
            if(StringUtils.isNotBlank(order.getField())){
    ...
    public Response transfer(@PathParam("orderId") String orderId, TransferOrderRequest request) {
    ...
    public Response statement(@PathParam("orderId") String orderId, StatementOrderRequest request) {
        Response response = new Response();
    ...
    @POST
    @Path("refund/{orderId}")
    public Response refund(@PathParam("orderId") String orderId, RefundOrderRequest request) {
        Response response = new Response();
