...
public class RedisCommonImpl extends AbstractRedisDAO<MainStationKeyEO> implements IRedisCommon {
    ...
    @Override
    public void incrInDay(String key, String url) {
        if (!url.contains("seller/repository/batchUpdate") && !url.contains("seller/repository/add") && !url.contains("seller/repository/batchUpdateForRob") && !url.contains("seller/repository/upload") && !url.contains("seller/repository/addrepository")) {
            return;
        }
        String staticKey = key + ":" + formatterDay.format(new Date());
        Long count = valueOps.increment(staticKey, 1);
        if (count == 1) {
            redisTemplate.expire(staticKey, 60 * 60 * 24 * 3, TimeUnit.SECONDS);
        }
    }
