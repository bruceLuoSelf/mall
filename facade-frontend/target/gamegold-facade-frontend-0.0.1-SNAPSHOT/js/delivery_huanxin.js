...
var receivedMsg = function (message) { //收到文本消息
    ...
    var tp = 'hx_sh';
    if (message.ext && message.ext.userType == 'sellerSystem') {
        tp = 'hx_sys';
    } else if (message.ext && message.ext.userType == 'seller') {
        tp = 'hx_my';
    } else if (message.ext && message.ext.userType == 'buyerSystem') {
        return;
    }
    var time = formatDateTime(new Date());
    var cont = myMsgCont(msgCont, tp, time);
    //TODo 环信的数据传输
    // alert('环信'+orderLog.id);

    if (message.ext.id != null && tp == 'hx_sys' && sysHxIds.contains(Number(message.ext.id))) {

        return;
    }
    console.log('环信服务器' + message.data);
    $('.hxMsgC').append(cont);
    toobottom();
}
        ...
        success: function (resp) {
            ...
            if (code == "00") {
                ...
                for (var i in list) {
                    ...
                    // alert('网络'+orderLog.id);
                    //TODO 环信的判断
                    // if(sysHxIds.contains(orderLog.id)){
                    //     return;
                    // }
                    // if(orderLog.id==null){
                    //     return;
                    // }
                    sysHxIds.push(orderLog.id);
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            if (code == "00") {
                ...
                options.user = deliveryOrder.sellerHxAccount;
                options.pwd = deliveryOrder.sellerHxPwd;
